...
class Form(BaseComponent):
    ...
    def datiImbarcazione(self,pane):
        fb = pane.div(margin_left='50px',margin_right='80px').formbuilder(cols=2, border_spacing='4px')
        #fb = pane.formbuilder(cols=2, border_spacing='4px')
        fb.field('tip_imbarcazione_code', width='25em' )
        fb.field('nome' , width='25em')
        fb.field('flag',columns='$nome,$code',auxColumns='$nome', limit=20, width='25em' )
        fb.field('imo',validate_onAccept='FIRE #FORM.imo' ,values='N/A,TBA',hasArrowDown=True,tag='comboBox',placeholder='select N/A or TBA or insernt relative IMO number', width='25em')
        #fb.field('loa',validate_regex=" ^[0-9,]*$",validate_regex_error='Insert only numbers and comma', placeholder='eg: 10 or 10,50' )
        fb.field('loa', width='40%' )
        fb.br()
        #fb.field('loa', placeholder='eg: 10 or 10,50',validate_onAccept='FIRE #FORM.loa' )
        fb.field('gt', width='40%' )
        fb.br()
        #fb.field('gt',validate_regex=" ^[0-9,]*$",validate_regex_error='Insert only numbers and comma', placeholder='eg:1200 or 1200,00' )
        fb.field('nt', width='40%' )
        #fb.field('nt' ,validate_regex=" ^[0-9,]*$",validate_regex_error='Insert only numbers and comma', placeholder='eg:650 or 650,00')
